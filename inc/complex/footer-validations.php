		...
		if( -1 != $this->wp_footer_line && -1 != $this->body_close_tag_line ) {
			...
			if( $diff < 0 || $diff > 1 ) {
				...
				if( $diff > 2 ||
					...
					$error_text = __( 'wp_footer call should be right before the closing body tag.', 'wp_theme_mentor_envato' );
			...
			$error_text = __( 'No wp_footer or closing body tag found', 'wp_theme_mentor_envato' );
