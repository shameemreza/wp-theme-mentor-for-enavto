	...
	private function does_wp_title_exist( $file ) {
		...
		if ( !empty( $lines_found ) && is_array( $lines_found ) ) {
			foreach ( $lines_found as $line => $snippet ) {
				...
				$error_text				 = __( 'Title tag found in header.php, Please add add_theme_support( \'title-tag\' ) in after_setup_theme action hook.', 'wp_theme_mentor_envato' );
		...
		if ( -1 != $this->wp_head_line && -1 != $this->head_close_tag_line ) {
			...
			if ( $diff < 0 || $diff > 1 ) {
				...
				false === strpos( $this->file[ $this->head_close_tag_line - 1 ], '?>' ) ) {
					$error_text = __( 'wp_head call should be right before the closing head tag.', 'wp_theme_mentor_envato' );
			...
			$error_text				 = __( 'No wp_head or closing head tag found', 'wp_theme_mentor_envato' );
