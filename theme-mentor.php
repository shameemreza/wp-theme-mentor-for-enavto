...
/**
 * Plugin Name: WP Theme Mentor For Envato
 * Description: WP Theme Mentor is a cousing of the Theme-Check plugin getting deeper into the code analysis.
 ...
 * Plugin URI: https://github.com/shameemreza/wp-theme-mentor-for-enavto
 * Version: 0.1
 * Author:  Shameem Reza
...
class Mentor_Themeforest {
	...
	public function __construct() {
		...
		add_action( 'wp_theme_mentor_envato_before_tests_list', array( $this, 'display_theme_name_tested' ) );
		// TODO: temporary
// 		$this->do_everything();
	}
	...
	public function theme_mentor_page_cb() {
		if ( !current_user_can( 'manage_options' ) ) {
			wp_die( __( 'We all know you shouldn\'t be here', 'wp_theme_mentor_envato' ) );
		}
		...
		echo '<h2>' . __( 'Theme Mentor For Themeforest', 'wp_theme_mentor_envato' ) . '</h2>';
		...
		do_action( 'wp_theme_mentor_envato_before_admin_page' );
		include_once 'inc/templates/admin-template.php';
		do_action( 'wp_theme_mentor_envato_after_admin_page' );
		echo '</div>';
		...
		do_action( 'wp_theme_mentor_envato_before_tests_list' );
	...
	public function display_theme_name_tested() {
		if ( !empty( $_POST[ 'dx_theme' ] ) ) {
			$testing_word = __( 'Testing', 'wp_theme_mentor_envato' );
